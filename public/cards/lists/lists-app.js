...
export class ListsApp {
    ...
    async syncSingleList(listId) {
        try {
            this.ui.showLoading('Sincronizando lista...');
            // Note: Individual list sync would need to be implemented in lists-sync.js
            await this.syncAllLists(); // For now, sync all
            this.ui.hideLoading();
            this.ui.showSuccess('Lista sincronizada com sucesso!');
            return true;
        } catch (error) {
            console.error('Error syncing list:', error);
            this.ui.hideLoading();
            this.ui.showError('Erro ao sincronizar lista');
            return false;
        }
    }
