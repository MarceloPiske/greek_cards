/**
 * Modal functionality for vocabulary cards system
 */
...
import { getStatusLabel } from './ui-utils.js';
...
export function showNewListModal() {
    ...
    createBtn.addEventListener('click', async () => {
        try {
            ...
            // Reload UI - need to import from main UI module
            const { loadWordLists, selectWordList } = await import('./ui.js');
            await loadWordLists();
 ...
 */
export function showAddWordModal(currentListId) {
    // Create modal HTML
    const modalHtml = `
    ...
    saveBtn.addEventListener('click', async () => {
        try {
            const { addVocabularyWord } = await import('./vocabulary.js');
            const { loadVocabularyWords, loadWordListContent } = await import('./ui.js');
            ...
            if (addToCurrentList && addToCurrentList.checked && currentListId) {
                await addWordsToList(currentListId, [newWord.id]);
            }
            ...
            if (addToCurrentList && addToCurrentList.checked && currentListId) {
                await loadWordListContent(currentListId);
            }
        } catch (error) {
...
export async function showAddToListModal(currentListId) {
    try {
        ...
        const { getSystemVocabulary } = await import('./vocabulary.js');
        
        allWords = await getSystemVocabulary({
            sortByStatus: true,
        });
        loadingModal.remove();
        activeFilter = 'all';
        activeSearchQuery = '';
        currentPage = 1;
        renderAddWordsModal(currentListId);
...
function filterWords(words, category, query) {
    return words.filter(w => {
        const inCategory = category === 'all' || (w.categoria && w.categoria.toLowerCase().includes(category));
        const matchesQuery = query === '' || [
            w.grego, w.translit, w.significado
        ].some(f => f && f.toLowerCase().includes(query.toLowerCase()));
        return inCategory && matchesQuery;
    });
}

function renderAddWordsModal(currentListId) {
    const filteredWords = filterWords(allWords, activeFilter, activeSearchQuery);
    const PAGE_SIZE = 100;
    ...
    modal.innerHTML = buildAddWordsModalContent(pagedWords, currentPage, totalPages);
    ...
    setupFilterHandlers(modal, (newFilter) => {
        activeFilter = newFilter;
        currentPage = 1;
        renderAddWordsModal(currentListId);
    });
    setupModalActions(modal, selectedWordIds, addBtn, currentListId);
...
function setupFilterHandlers(modal, onFilterChange) {
...
function buildAddWordsModalContent(wordsPage, currentPage = 1, totalPages = 1) {
    const filtersHtml = Object.values(WordCategories).map(c =>
        `<button class="filter-btn" data-filter="${c}">${c}</button>`).join('');

    const wordsHtml = wordsPage.map(word => `
        <div class="vocab-word-item ${word.progress?.status || 'unread'}" data-word-id="${word.ID}" data-category="${word.PART_OF_SPEECH}">
            ...
            <div class="word-info">
                ...
                <div class="word-meta">
                    ...
                    <span class="status-badge ${word.progress?.status || 'unread'}">${getStatusLabel(word.progress?.status || 'unread')}</span>
                </div>
            </div>
        </div>`).join('');
    ...
    return `
        <div class="modal-content large">
            ...
            <div class="search-filter-bar">
                <input type="text" id="modal-search" placeholder="Buscar palavras..." class="search-input">
...
function setupModalActions(modal, selectedWordIds, addBtn, currentListId) {
    addBtn.addEventListener('click', async () => {
        if (!selectedWordIds.size) return showToast('Selecione palavras para adicionar');
        try {
            await addWordsToList(currentListId, Array.from(selectedWordIds));
            showToast(`${selectedWordIds.size} palavras adicionadas com sucesso!`);
            modal.remove();
            
            const { loadWordListContent } = await import('./ui.js');
            await loadWordListContent(currentListId);
        } catch (error) {
            console.error(error);
 ...
 */
export function confirmRemoveFromList(currentListId, selectedWords) {
    if (!currentListId || selectedWords.length === 0) return;
    ...
    confirmBtn.addEventListener('click', async () => {
        try {
            await removeWordsFromList(currentListId, selectedWords);
            showToast(`${selectedWords.length} palavras removidas com sucesso!`);
            modal.remove();
...
export async function showEditListModal(listId) {
    try {
        ...
        saveBtn.addEventListener('click', async () => {
            try {
                const { initVocabularyDB } = await import('./vocabulary.js');
                const { loadWordLists, loadWordListContent } = await import('./ui.js');
 ...
 */
export async function confirmDeleteList(listId, uiState) {
    // Create modal HTML
    const modalHtml = `
    ...
    confirmBtn.addEventListener('click', async () => {
        try {
            // Delete list
            const { initVocabularyDB } = await import('./vocabulary.js');
            const { loadWordLists, showEmptyState } = await import('./ui.js');
                ...
                <div class="modal-content">
                    ...
                    <div class="word-detail-section">
                        ...
                        <div class="detail-row">
                            ...
                            <div class="detail-value">${word.pronuncia || '-'}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Significado:</div>
                            <div class="detail-value">${word.USAGE || '-'}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Definição:</div>
                            <div class="detail-value">${word.definicaoCompleta || word.USAGE || '-'}</div>
                        </div>
                        ${word.origem ? `
                        <div class="detail-row">
                            <div class="detail-label">Origem:</div>
                            <div class="detail-value">${word.origem}</div>
                        </div>` : ''}
                    </div>
...
export async function startPracticeSession(listId) {
    try {
        const { getWordListWithWords, updateWordProgress, WordStatus } = await import('./vocabulary.js');
            ...
            cardContainer.innerHTML = `
                <div class="practice-card ${word.progress.status}" data-word-id="${word.ID}">
                    <div class="card-front">
                        ...
                        ${word.pronuncia || word.origem || word.definicaoCompleta ? `
                    ...
                    <div class="card-back">
                        <div class="meaning">${word.USAGE}</div>
