...
export class ListsUI {
    ...
    createListCard(list) {
        ...
        const isOffline = !list.syncedAt || (list.updatedAt && list.updatedAt > list.syncedAt);
        const canSync = getCurrentUserPlan() !== 'free';

        return `
            <div class="list-card ${isOffline && canSync ? 'offline' : ''}" data-list-id="${list.id}">
                <div class="sync-status ${isOffline && canSync ? 'offline' : canSync ? 'synced' : 'local'}"></div>
                ...
                <div class="list-actions">
                    ...
                    ${isOffline && canSync ? `
                    <button class="btn sync-single-btn" data-list-id="${list.id}" title="Esta lista não está sincronizada">
